...
# 파일 저장 경로
dir_path = 'C:/SGIS/QGIS/전국 교육서비스업 행정동별 분석(PyQGIS)/'
...
path_stat_cp = dir_path + '2025년기준_2024년_산업분류별(11차_중분류)_사업체수.csv'
uri_stat_cp = f'file:///{path_stat_cp}?type=csv&useHeader=no&geomType=none&encoding=UTF-8'
stat_cp = QgsVectorLayer(uri_stat_cp, '2024년_산업분류별(11차_중분류)_사업체수', 'delimitedtext')
...
# 선택한 피처를 다른 이름으로 저장
path_stat_edu = dir_path + "2024년_교육서비스업(11차_중분류)_사업체수.csv"
...
stat_cp_edu = QgsVectorLayer(uri_stat_edu, '2024년_교육서비스업(11차_중분류)_사업체수', 'delimitedtext')
...
sgg_line = QgsSimpleLineSymbolLayer(width=0.26, color=QColor('black')) # 심볼 레이어 생성
...
layout.setName("전국 교육서비스업 행정동별 분석") # 조판 이름
...
map_item.attemptMove(QgsLayoutPoint(15, 15)) # 지도 왼쪽 위 모서리 위치 설정
map_item.attemptResize(QgsLayoutSize(width - 30, height - 30)) # 지도 너비, 높이
layer = QgsProject.instance().mapLayersByName("bnd_dong_00_2025_2Q")[0]
...
title.setText("전국 교육서비스업 행정동별 사업체수(2024)")  # 제목 텍스트
title.setFont(QFont("맑은 고딕", 25, QFont.Bold)) # 폰트 설정
title.adjustSizeToText()  # 텍스트 크기에 맞게 자동 조정
title.attemptMove(QgsLayoutPoint(15, 10))  # 제목 위치(좌측 상단)
...
# 범례 위치(우측 하단)
legend.attemptMove(QgsLayoutPoint(width - 100, height - 80))
...
path_png = dir_path + '전국 교육서비스업 행정동별 분석.png'
