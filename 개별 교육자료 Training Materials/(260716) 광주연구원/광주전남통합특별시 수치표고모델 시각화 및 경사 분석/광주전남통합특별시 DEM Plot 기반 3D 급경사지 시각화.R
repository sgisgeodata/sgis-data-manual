...
# 광주+전남은 면적이 넓으므로 광주 단독보다 크게 설정
# 너무 무거우면 20000, 더 자세히 보고 싶으면 50000으로 조정
target_cells <- 30000

# 화살표 최소 후보 기준
# HTML 버튼에서 15/20/25/30도를 선택할 수 있도록 최소 후보는 15도 이상으로 생성
arrow_min_degree <- 15
...
gwangju_jeonnam <- sigungu %>%
  mutate(SIGUNGU_CD = as.character(SIGUNGU_CD)) %>%
  filter(substr(SIGUNGU_CD, 1, 2) %in% c("24", "36"))

if (nrow(gwangju_jeonnam) == 0) {
  stop("SIGUNGU_CD 기준으로 광주+전남 경계를 찾지 못했습니다.")
}

cat("선택된 광주+전남 시군구 목록:\n")
print(
  gwangju_jeonnam %>%
    st_drop_geometry() %>%
    select(SIGUNGU_CD, SIGUNGU_NM)
)
...
gwangju_jeonnam_vect <- vect(gwangju_jeonnam)


# 5. DEM 자르기 -------------------------------------------------

dem_gwangju_jeonnam <- dem %>%
  crop(gwangju_jeonnam_vect) %>%
  mask(gwangju_jeonnam_vect)

dem_gwangju_jeonnam[dem_gwangju_jeonnam < -100] <- NA
...
if (ncell(dem_gwangju_jeonnam) > target_cells) {
  fact_value <- ceiling(sqrt(ncell(dem_gwangju_jeonnam) / target_cells))
...
dem_geojson_text <- paste(
  readLines(temp_geojson, encoding = "UTF-8"),
  collapse = "\n"
)
...
# 핵심:
# - 화살표는 화면에 보이는 dem_plot 기준으로 계산한다.
# - 각 셀 A에서 주변 8개 이웃 B 중 가장 급하게 낮아지는 B를 찾는다.
# - A가 B보다 높을 때만 하향 후보로 본다.
# - A와 B가 선으로 접하면 A의 B쪽 윗면 모서리 중심에서 시작한다.
# - A와 B가 점으로 접하면 A의 B쪽 윗면 꼭짓점에서 시작한다.
# - 끝점은 B 셀의 윗면 중심이다.
# - 시작점과 도착점이 완전히 같은 동일 화살표만 중복 제거한다.
...
# 도착점이 같아도 시작점이 다르면 유지
# 시작점이 같아도 도착점이 다르면 유지
...
gwangju_jeonnam_4326 <- st_transform(gwangju_jeonnam, 4326)

gwangju_jeonnam_boundary <- st_cast(
  gwangju_jeonnam,
  "MULTILINESTRING",
  warn = FALSE
)
...
mean_elev <- global(dem_gwangju_jeonnam, mean, na.rm = TRUE)[1, 1]

for (i in seq_len(nrow(gwangju_jeonnam_boundary))) {
  
  line_coords <- st_coordinates(gwangju_jeonnam_boundary[i, ])
  
  # 중요:
  # MULTILINESTRING은 여러 개의 선 조각으로 구성될 수 있음.
  # L1, L2, L3 그룹을 무시하면 떨어진 섬과 해안선이 긴 직선으로 연결됨.
  # 따라서 선 조각별로 나누어 각각 PathLayer에 넣어야 함.
  
  coord_df <- as.data.frame(line_coords)
  
  group_cols <- intersect(c("L1", "L2", "L3"), names(coord_df))
  
  if (length(group_cols) == 0) {
    coord_df$line_group <- 1
  } else {
    coord_df$line_group <- interaction(
      coord_df[, group_cols, drop = FALSE],
      drop = TRUE
    )
  }
  
  split_lines <- split(coord_df, coord_df$line_group)
  ...
  for (one_line in split_lines) {
    ...
    boundary_pts <- st_as_sf(
      ...
      coords = c("x", "y"),
      crs = crs(dem_gwangju_jeonnam)
    )
    
    elev_vals <- terra::extract(dem_gwangju_jeonnam, vect(boundary_pts))[, 2]
    ...
    boundary_data[[boundary_index]] <- list(
      name = gwangju_jeonnam$SIGUNGU_NM[i],
...
center <- st_centroid(st_union(gwangju_jeonnam_4326))
...
<head>
<meta charset="utf-8"/>
<title>광주전남특별시 DEM Plot 기반 3D 급경사지 시각화</title>
...
<div id="title">
  광주전남특별시 DEM Plot 기반 3D 급경사지 시각화
  <div id="subtitle">광주·전남 통합 권역의 지형 고저 차이와 급경사지 하향 흐름 방향 표시</div>
</div>
