...
# 2. 강원도 경계와 겹치는 격자 경계 만들기

# 2-1. 강원도 시도 경계
file_path <- paste(prj_dir, "경계", "bnd_sido_32_2025_2Q.shp", sep="/")
bord_sido <- st_read(file_path, options="ENCODING=UTF-8") # 경계 읽어들이기, geometry 컬럼에 공간정보 포함

# 2-2. 강원도 시군구 경계
file_path <- paste(prj_dir, "경계", "bnd_sigungu_32_2025_2Q.shp", sep="/")
bord_sigungu <- st_read(file_path, options="ENCODING=UTF-8")

# 2-3. 강원도 행정동 경계
file_path <- paste(prj_dir, "경계", "bnd_dong_32_2025_2Q.shp", sep="/")
bord_dong <- st_read(file_path, options="ENCODING=UTF-8")

# 2-4. 강원도를 포함하는 1km 격자 경계(다아, 라바, 라사, 라아, 마바, 마사, 마아)
file_path <- paste(prj_dir, "경계", "grid_다아_1K.shp", sep="/")
bord_grid_1 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_라바_1K.shp", sep="/")
bord_grid_2 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_라사_1K.shp", sep="/")
bord_grid_3 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_라아_1K.shp", sep="/")
bord_grid_4 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_마바_1K.shp", sep="/")
bord_grid_5 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_마사_1K.shp", sep="/")
bord_grid_6 <- st_read(file_path)
file_path <- paste(prj_dir, "경계", "grid_마아_1K.shp", sep="/")
bord_grid_7 <- st_read(file_path)

bord_grid_gangwon <- rbind(bord_grid_1, bord_grid_2, bord_grid_3, bord_grid_4, bord_grid_5, bord_grid_6, bord_grid_7)

# 2-4. 강원도 경계와 겹치는 격자 경계만 저장
bord_intersects <- 
  st_join(bord_grid_gangwon, bord_sido, join=st_intersects, left=FALSE)      

# 강원도와 겹치는 격자 경계를 지도에서 확인
...
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_다아_1K.csv", sep="/")
stat_1 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
# 한글을 포함한 통계 파일의 인코딩을 'CP949'로 지정
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_라바_1K.csv", sep="/")
stat_2 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_라사_1K.csv", sep="/")
stat_3 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_라아_1K.csv", sep="/")
stat_4 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_마바_1K.csv", sep="/")
stat_5 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_마사_1K.csv", sep="/")
stat_6 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2019년_사업체중분류_마아_1K.csv", sep="/")
stat_7 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")

stat <- rbind(stat_1, stat_2, stat_3, stat_4, stat_5, stat_6, stat_7)
...
# 3-2. 2023년
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_다아_1K.csv", sep="/")
stat_1 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
# 한글을 포함한 통계 파일의 인코딩을 'CP949'로 지정
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_라바_1K.csv", sep="/")
stat_2 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_라사_1K.csv", sep="/")
stat_3 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_라아_1K.csv", sep="/")
stat_4 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_마바_1K.csv", sep="/")
stat_5 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_마사_1K.csv", sep="/")
stat_6 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
file_path <- paste(prj_dir, "통계", "2023년_사업체중분류_마아_1K.csv", sep="/")
stat_7 <- read.csv(file=file_path, header=FALSE, fileEncoding="CP949")
...
join_2019 <- join_2019 %>% select(-BASE_DATE, -SIDO_CD, -SIDO_NM, -STAT_CODE)
...
# 불필요한 컬럼 정리, 기준연도(BASE_YEAR) NULL값 채우기
join_2023 <- join_2023 %>% select(-BASE_DATE, -SIDO_CD, -SIDO_NM, -STAT_CODE)
join_2023$BASE_YEAR <- "2019"
...
# 불필요한 컬럼 정리, 기준연도(BASE_YEAR) NULL값 채우기
join_comp <- join_comp %>% select(-BASE_DATE, -SIDO_CD, -SIDO_NM)
join_comp[is.na(join_comp$DIFF), "DIFF"] <- 0
str(join_comp)
max(join_comp$DIFF) # 차이값 최대값 21
min(join_comp$DIFF) # 차이값 최소값 -23
...
map_2019 <- ggplot() +
  ...
  geom_sf_text(data=bord_sigungu, mapping=aes(label=SIGUNGU_NM), alpha=0.7, size=3) +
  # 4. 강원도 시도 경계
  geom_sf(data=bord_sido, color="black", fill=NA, linewidth=0.2) + 
  # 5. 제목과 테마 지정
  ggtitle("2019년 강원도 숙박업") +
...
# 통계값의 구간 범위 컬럼 만들기
join_comp$BREAKS <- 
  cut(join_comp$DIFF, breaks = c(-23, -15, -10, -1, 1, 10, 15, 21),          # 수정
      labels = c("-23~15","-15~-10", "-10~-1","-1~1","1~10", "10~15","15~21")) # 수정

map_comp <- ggplot() +
  # 1. 격자 경계(전체 순위)
  geom_sf(data=join_comp, mapping=aes(fill=BREAKS), linetype = "dotted") +
  scale_fill_manual(values=c("#08519C","#6BAED6","#BDD7E7","#FFFFFF","#FECC5C","#FD8D3C","#BD0026"),
  ...
  geom_sf(data=bord_sido, color="black", fill=NA, linewidth=0.2) + 
  # 5. 제목과 테마 지정
  ggtitle("2019년~2023년 강원도 숙박업 변화") +
  theme_bw() + # 축 제목, 축 텍스트 등 제거
  theme(axis.title=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), panel.grid=element_blank())
...
# 6-1. 지도를 그림파일로 저장
file_path <- paste(prj_dir, "2019년 강원도 숙박업 격자별 단계구분도.png", sep="/")
ggsave(map_2019, filename=file_path)
# 너비, 높이는 인치 단위로 지정 가능
file_path <- paste(prj_dir, "2023년 강원도 숙박업 격자별 단계구분도.png", sep="/")
ggsave(map_2023, filename=file_path)
file_path <- paste(prj_dir, "2019년~2023년 강원도 숙박업 변화 단계구분도.png", sep="/")
ggsave(map_comp, filename=file_path)


# 6.2. 경계를 SHP 파일로 저장
file_path <- paste(prj_dir, "2019년 강원도 숙박업 격자별 단계구분도.shp", sep="/")
st_write(join_2019, dsn=file_path, append=FALSE, layer_options="ENCODING=UTF-8")
# 한글을 포함한 파일을 저장할 때 적절한 인코딩 지정('UTF-8' 또는 'CP949' 등)
file_path <- paste(prj_dir, "2023년 강원도 숙박업 격자별 단계구분도.shp", sep="/")
st_write(join_2023, dsn=file_path, append=FALSE, layer_options="ENCODING=UTF-8")
file_path <- paste(prj_dir, "2019년~2023년 강원도 숙박업 변화 단계구분도.shp", sep="/")
