...
map_all <- ggplot() +
  ...
  scale_fill_manual(values=c("#FFFFB2","#FECC5C","#FD8D3C", "#F03820"),
                    na.value="white", na.translate=FALSE, name="격자별 고령인구 비율") +
  # 2. 제주시 시도 경계
  geom_sf(data=bord_sido, color="black", fill=NA, linewidth=0.5) + 
  # 3. 제주시 시군구 경계와 이름
  geom_sf(data=bord_sigungu, color="black", fill=NA, linewidth=0.3) +
  # 4. 제주시 행정동 경계
