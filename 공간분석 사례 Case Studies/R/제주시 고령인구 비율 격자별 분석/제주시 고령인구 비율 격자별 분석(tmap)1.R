...
map <- 
  tm_shape(bord_sido) + tm_borders(lty="solid", lwd=1.0, col="black")
...
# 테스트 시작(제주시 시군구 경계)
map <- 
  tm_shape(bord_sigungu) + tm_borders(lty="solid", lwd=1.0, col="black")
...
# 테스트 시작(제주시 행정동 경계)
map <- 
  tm_shape(bord_dong) + tm_borders(lty="solid", lwd=1.0, col="black")
...
map <- 
  tm_shape(bord_grid_jeju) + tm_borders(lty="solid", lwd=1.0, col="black") +
  tm_shape(bord_sido) + tm_borders(lty="solid", lwd=1.0, col="red")
...
map <- 
  tm_shape(bord_intersects) + tm_borders(lty="solid", lwd=1.0, col="black") +
  tm_shape(bord_sido) + tm_borders(lty="solid", lwd=1.0, col="red")
...
map_all <- 
  ...
  tm_polygons(fill="ratio_65p", lty="dotted", 
              ...
              fill.scale=tm_scale_intervals(breaks=c(0.01, 0.07, 0.14, 0.20, 0.91),    # 수정
                                            labels=c("0.01~0.07","0.07~0.14","0.14~0.20","0.20~0.91"), # 수정
                                            values=c("#FFFFB2","#FECC5C","#FD8D3C","#F03B20"), # 색상 코드 열거
...
# 지도를 그림파일로 저장
file_path <- paste(prj_dir, "2024년 제주도 고령인구 비율 격자별 분석.png", sep="/")
tmap_save(tm=map_all, filename=file_path, width=1800, height=1500, dpi=300)
# 너비, 높이(픽셀 단위), 해상도 지정


# 경계를 SHP 파일로 저장(전체 격자, 상위순위 격자)
file_path <- paste(prj_dir, "2024년 제주도 고령인구 비율 격자별 분석.shp", sep="/")
