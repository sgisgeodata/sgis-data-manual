...
jeju_elderly_ratio_map <- function(base_year, prj_dir="C:/SGIS/R/Proportion of Elderly Population by Grid in Jeju") {
  ...
  map_all <- ggplot() +
    ...
    # 5. 제목과 테마 지정
    ggtitle(paste0(base_year, "년 제주도 격자별 고령인구 비율")) +
    theme_bw() + # 축 제목, 축 텍스트 등 제거
    theme(axis.title=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), 
          legend.position = "none") # 범례 위치 조정(right) 또는 숨기기(none)
