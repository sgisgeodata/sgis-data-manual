...
map_all <- ggplot() +
  ...
  # 5. 제목과 테마 지정
  ggtitle("2023 Proportion of Elderly Population by Grid in Jeju") +
...
# 지도를 그림파일로 저장
file_path <- paste(prj_dir, "2024 Proportion of Elderly Population by Grid in Jeju.png", sep="/")
ggsave(map_all, filename=file_path)
# 너비, 높이는 인치 단위로 지정 가능

# 경계를 SHP 파일로 저장(전체 격자, 상위순위 격자)
file_path <- paste(prj_dir, "2024 Proportion of Elderly Population by Grid in Jeju.shp", sep="/")
